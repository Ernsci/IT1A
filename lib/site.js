...
async function getGroupPhoto() {
  ...
  try {
    ...
    console.error('getGroupPhoto failed:', err.message);
    return null;
  }
}

async function saveGroupPhoto(value) {
  const { error } = await supabase
    .from('settings')
    .upsert({ key: 'groupPhoto', value, updated_at: new Date().toISOString() }, { onConflict: 'key' });
  if (error) throw error;
}

module.exports = { getSite, saveSite, getGroupPhoto, saveGroupPhoto, DEFAULT_SITE };
